...
function openPicker(type) {
    ...
    const items = isLang
        ...
        : currentEpisode.langs[activeLang].servers.map((s, i) => {
             ...
             if (s.sandbox) {
                 extra = "Híbrido";
                 extraClass = "hibrido";
             } else if (url.includes(".m3u8") || url.includes(".mp4") || url.includes("directo")) {
                 extra = "Tipo Premium";
                 extraClass = "premium";
    ...
    items.forEach(it => {
        ...
        if (it.extra) {
            ...
            else if (it.extraClass === "premium") { badgeBg = "rgba(0, 230, 118, 0.15)"; badgeColor = "#00e676"; }
        ...
        <button class="vp-picker-opt" data-idx="${it.idx}" style="background:${bg};border:${border};color:${textColor};padding:14px 16px;border-radius:14px;display:flex;align-items:center;cursor:pointer;text-align:left;font-family:inherit;font-size:15px;font-weight:600;transition:all 0.2s;width:100%">
          ...
          ${extraBadge}
          ${isSelected ? `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="margin-left:12px;flex-shrink:0"><polyline points="20 6 9 17 4 12"></polyline></svg>` : ""}
        </button>
        `;
    });
